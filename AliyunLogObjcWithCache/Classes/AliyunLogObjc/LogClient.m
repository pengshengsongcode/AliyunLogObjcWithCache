...
@property (nonatomic, strong)CacheCheckManager *cacheManager;
...
- (id)initWithApp:(NSString*) endPoint accessKeyID:(NSString *)ak accessKeySecret: (NSString *)as projectName: (NSString *)name serializeType: (AliSLSSerializer) sType {
    if(self = [super init]) {
        ...
        _cacheManager = [[CacheCheckManager alloc] initWithTimeInterval:30 client:self fetchCount:30];
        
        [_cacheManager startCacheCheck];
...
- (void)PostLog:(RawLogGroup*)logGroup logStoreName:(NSString*)name call:(void (^)(NSURLResponse* _Nullable response,NSError* _Nullable error) )errorCallback {
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        [self HttpPostRequest: httpUrl withHeaders:httpHeaders andBody:httpPostBodyZipped callback:^(NSURLResponse * _Nullable response, NSError * _Nullable error) {
            ...
            if (error != nil || httpResponse.statusCode != 200) {
                ...
                [[DBManager sharedInstance] insertRecordsWithEndpoint:self.GetEndPoint project:_mProject logstore:name log:[logGroup jsonPachage] timestamp:time];
                
                DLog(@"阿里云埋点 - 失败 - \nstatusCode:%ld,\nerrorCode:%ld,\nerrMessage:%@",httpResponse.statusCode,error.code,error.domain);
            }else {
                DLog(@"阿里云埋点 - 成功");
            }
...
- (void)PostLogInCache:(RawLogGroup*)logGroup logStoreName:(NSString*)name call:(void (^)(NSURLResponse* _Nullable response,NSError* _Nullable error) )errorCallback {
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        // Force to use https api interface
        // Due to the requirement of Apple Security Policy after Jan. 1st, 2017
        NSString *httpUrl = [NSString stringWithFormat:@"https://%@.%@/logstores/%@/shards/lb",_mProject,_mEndPoint,name];
